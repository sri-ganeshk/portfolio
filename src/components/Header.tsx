    ...
    >
      <div className="container-custom flex justify-between items-center">
        ...
        >
          ...
          <span className="font-mono text-lg tracking-wider">SG</span>
        </motion.div>

        <nav className="hidden md:block">
          <ul className="flex space-x-8">
            {navItems.map((item, i) => (
              <motion.li key={item} custom={i} variants={navItemVariants} initial="hidden" animate="visible">
                <a href={`#${item.toLowerCase()}`} className="text-sm font-medium hover:text-light transition-colors duration-300">
                  {item}
                </a>
              </motion.li>
            ))}
        ...
        >
          <div className="w-6 flex flex-col space-y-1.5">
            <motion.div className="w-full h-px bg-light" animate={mobileMenuOpen ? { rotate: 45, y: 7 } : { rotate: 0, y: 0 }} transition={{ duration: 0.3 }} />
            <motion.div className="w-full h-px bg-light" animate={mobileMenuOpen ? { opacity: 0 } : { opacity: 1 }} />
            <motion.div className="w-full h-px bg-light" animate={mobileMenuOpen ? { rotate: -45, y: -7 } : { rotate: 0, y: 0 }} transition={{ duration: 0.3 }} />
          </div>
        </motion.button>
              ...
              <ul className="flex flex-col space-y-4">
                {navItems.map(item => (
                  <motion.li key={item} whileHover={{ x: 5 }} transition={{ duration: 0.2 }}>
                    <a href={`#${item.toLowerCase()}`} className="text-sm font-medium" onClick={() => setMobileMenuOpen(false)}>
                      {item}
                    </a>
                  </motion.li>
                ))}
